...
$mensaje = ''; // Not used directly, but kept for compatibility if needed
$error = $_GET['error'] ?? '';
$success = $_GET['success'] ?? '';

// Filtro de sala (opcional, por GET)
$filtro_sala_id = $_GET['sala_id'] ?? null;

// --- LÓGICA DE BÚSQUEDA DE MESAS ---
...
if ($busqueda_realizada) {
    ...
    // Duración estándar 1h30m, Privadas turno completo
    $fecha_fin_normal = date('Y-m-d H:i:s', strtotime('+90 minutes', strtotime($fecha_inicio_str)));
    
    // Definir turnos para privadas (Comida hasta 17:00, Cena desde 20:00)
    if ($hora < '17:00') {
        $fecha_fin_privada = "$fecha 17:00:00";
    } else {
        $fecha_fin_privada = "$fecha 23:59:59";
    }
...
// --- OBTENER LISTADO DE RESERVAS FUTURAS ---
$sql_reservas = "
...
// Obtener nombre de sala si hay filtro
$nombre_sala_filtro = '';
if ($filtro_sala_id) {
...
<body 
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 10px;">
                                            <!-- Editar (Datos en atributos para seguridad) -->
    ...
    <!-- Modal para editar reserva (Estructura igual a usuarios) -->
    ...
    <!-- Modal Confirmar Reserva (Creación) -->
